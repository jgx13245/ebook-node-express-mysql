...
router.get('/mybook', function (req, res, next) {
  ...
  adminmodel.selectMyBook(userid, function (err, chanpinList) {
    if (err) {
      return next(err);
    }
    res.render('admin/chanpin/mybook', {
      title: '我的书籍',
      chanpinList: chanpinList
    });
  });
...
router.post('/addChanPinInfo', function(req, res, next) {
  ...
  var userid = req.session.uid;
  adminmodel.addChanPinInfo(place, introduce, fenlei, userid, function(err, rows) {
    if (err) {
      res.json({
        'error': err
      });
...
router.post('/editChanPinInfo', function(req, res, next) {
  ...
  var userid = req.session.uid;
  var chanpinid = req.body.chanpinid;
  adminmodel.updateChanPinInfo(place, introduce, fenlei, userid, chanpinid, function(err) {
    if (err) {
      res.json({
        'error': err
      });
...
router.post('/addXingCheng', function(req, res, next) {
  ...
  adminmodel.addXingCheng(chanpinid, renshu, size, color, function(err) {
    if (err) {
      res.json({
        'error': err
      });
...
// 审核书籍
router.post('/shenheSj', function (req, res, next) {
  var chanpinid = req.body.chanpinid;
  adminmodel.shenheSj(chanpinid, function (err) {
    if (err) {
      res.json({
        'error': err
      });
