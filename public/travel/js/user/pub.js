...
$(document).on('click', '.btn-reg', function() {
  ...
  var usertype = $('select[name="reg-usertype"]').val();
  var data = {
    'account': account,
		'password': password,
		'name': name,
...
// 加入收藏
$(document).on('click', '.btn-scsj', function() {
...
// 取消收藏
$(document).on('click', '.btn-qxscsj', function() {
  var chanpinid = $(this).data('chanpinid');
  var data = {
    'chanpinid': chanpinid
  }
  ajaxPost('/qxShouCang', data, function(result) {
    if (result.success) {
      showTips('success', 'Success!', result.success);
      setTimeout(function() {
        location.reload();
